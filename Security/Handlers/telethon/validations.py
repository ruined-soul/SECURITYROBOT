from Security.Handlers.telethon import IMMUNE_USERS, telethn
from Security import REQUESTER
...
async def security_is_admin(chat_id: int):
    status = False
    yone = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if security.id == user.id:
            status = True
            break
    return status
